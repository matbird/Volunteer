...
//		LogUtils.i("test", "userinfo:"+currentUser.getArea()+" verify:"+currentUser.getVerify());
		...
		/*YGUser user = new YGUser();
		user.setScore(100);
		user.setSignature("这家伙很懒！");
		
		YGUser currentUser = BmobUser.getCurrentUser(this, YGUser.class);
		user.update(this,currentUser.getObjectId(),new UpdateListener() {
			@Override
			public void onSuccess() {
		...
		File f = new File(MatCacheUtils.getCacheDirectory(this, true,
				"pic") + dateTime);
		if (f.exists()) {
			f.delete();
		}
...
//					openLayout.setVisibility(View.GONE);
	...
	private void publish() {
		...
		figureFile.upload(this, new UploadFileListener() {
			...
			public void onSuccess() {
				// TODO Auto-generated method stub
				LogUtils.i(TAG,
						"上传文件成功。" + figureFile.getFileUrl(LoginActivity.this));
				publishWithoutFigure(figureFile);
	...
	private void publishWithoutFigure(final BmobFile figureFile) {
		YGUser user = new YGUser();
		user.setSignature("我上传了头像啦");
		if(figureFile != null)
			user.setPhoto(figureFile);
